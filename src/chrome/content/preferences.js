...
https_always = CC["@hyperbola.info/https-always;1"]
  .getService(Components.interfaces.nsISupports)
  .wrappedJSObject;

rulesets = [];

const pref_prefix = "extensions.https_always.";
const GITID = https_always.https_rules.GITCommitID;
...
function reset_defaults() {
  https_always.https_rules.resetRulesetsToDefaults()
  treeView.treebox.invalidate();
}
...
function viewXMLSource() {
  ...
  for (var t = 0; t < numRanges; t++){
    ...
    for (var v = start.value; v <= end.value; v++){
      ...
      aWin.openDialog("chrome://https-always/content/fetch-source.xul",
...
function getValue(row, col) {
  switch (col.id) {
    ...
    case "note_col":
      return row.notes;
    case "enabled_col":
      return https_always.https_rules.rulesetsByName[row.name].active;
      /*var ruleActive = false;
      try {
        if(https_always.rule_toggle_prefs.getBoolPref(row.name))
          ruleActive = true;
      } catch(e) {
        ruleActive = https_always.https_rules.rulesetsByName[row.name].active;
      }
      return ruleActive;*/
    default:
      return;
...
function https_prefs_init(doc) {
  ...
  https_always.https_rules.loadAllRulesets();
  rulesets = Array.slice(https_always.https_rules.rulesets);
