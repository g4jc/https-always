...
// Define https always variable object that will act as a namespace, so that 
...
httpsAlways.fetchSource = {
   ...
   */
  init: function() {
    var fs = httpsAlways.fetchSource;

    if("arguments" in window && window.arguments.length > 0) {
      var filename = window.arguments[0].xmlName;
      var URL = fs.getURL(filename);
  ...
  getURL: function(filename) {
    var fs = httpsAlways.fetchSource;
    return fs.BASE_SITE + fs.DIRECTORY + filename;
  },
...
// hook event for init
window.addEventListener("load", httpsAlways.fetchSource.init, false);
