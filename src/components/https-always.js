...
const SERVICE_CTRID = "@hyperbola.info/https-always;1";
...
const INCLUDE = function(name) {
  if (arguments.length > 1)
    ...
    LOADER.loadSubScript("chrome://https-always/content/code/"
            + name + ".js");
    _INCLUDED[name] = true;
  }
};
...
const OBSERVER_TOPIC_URI_REWRITE = "https-always-uri-rewrite";
...
function HTTPSAlways() {

  // Set up logging in each component:
  HTTPS.log = HTTPSRules.log = RuleWriter.log = this.log = https_alwaysLog;

  this.expandoMap = new WeakMap();

  this.log = https_alwaysLog;
  ...
  branch.addObserver("extensions.https_always.enable_mixed_rulesets",
                         this, false);
...
1. HTTPSAlways.shouldIgnoreURI() checks for very quick reasons to ignore a
...
HTTPSAlways.observe()    finds a reference to the ApplicableList or alist that represents the toolbar context menu
...
HTTPSAlways.prototype = {
  ...
  _xpcom_factory: {
    createInstance: function (outer, iid) {
      ...
      if (!HTTPSAlways.instance)
        HTTPSAlways.instance = new HTTPSAlways();
      return HTTPSAlways.instance.QueryInterface(iid);
    },
  ...
  shouldIgnoreURI: function(channel, alist) {
    ...
    if (uri.spec in https_always_blacklist) {
        ...
        if (alist) {
          alist.breaking_rule(https_always_blacklist[uri.spec]);
        } else {
  ...
  loadOCSPList: function() {
    try {
      var loc = "chrome://https-always/content/code/commonOCSP.json";
    ...
    if (topic == "http-on-modify-request") {
      ...
      this.browser_initialised = true;
        ...
        switch (data) {
            ...
            case "extensions.https_always.enable_mixed_rulesets":
  ...
  get_prefs: function(prefBranch) {
    ...
    if(prefBranch === PREFBRANCH_RULE_TOGGLE)
      branch_name = "extensions.https_always.rule_toggle.";
    else if (prefBranch === PREFBRANCH_NONE)
      branch_name = "";
    else
      branch_name = "extensions.https_always.";
    var o_prefs = false;
    var o_branch = false;
    // this function needs to be called from inside https_alwaysLog, so
    ...
    if (!o_prefs)
    {
      econsole.logStringMessage("HTTPS Always: Failed to get preferences-service!");
      return false;
    }
    ...
    if (!o_branch)
    {
      econsole.logStringMessage("HTTPS Always: Failed to get prefs branch!");
      return false;
    }
    ...
    if(prefBranch == PREFBRANCH_ROOT) {
      ...
      try {
        ...
        econsole.logStringMessage("Creating new about:config https_always.LogLevel variable");
  ...
  toggleHttpNowhere: function() {
    ...
    let thisBranch =
      prefService.getBranch("extensions.https_always.http_nowhere.");
...
var prefs = 0;
var econsole = 0;
function https_alwaysLog(level, str) {
  if (prefs == 0) {
    prefs = HTTPSAlways.instance.get_prefs();
  ...
  try {
    ...
    econsole.logStringMessage( "HTTPS Always: Failed to read about:config LogLevel");
    threshold = WARN;
  }
  if (level >= threshold) {
    var levelName = ["", "VERB", "DBUG", "INFO", "NOTE", "WARN"][level];
    var prefix = "HTTPS Always " + levelName + ": ";
...
if (XPCOMUtils.generateNSGetFactory)
    var NSGetFactory = XPCOMUtils.generateNSGetFactory([HTTPSAlways]);
else
    var NSGetModule = XPCOMUtils.generateNSGetModule([HTTPSAlways]);
